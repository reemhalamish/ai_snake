    ...
    def detect_laser(self, img):
        ...
        for i, row in enumerate(cpy):
            for j, val in enumerate(row):
                if val:
                    all_white.append((i,j))
        end = time()
        print("found {} items and it took {} ms".format(len(all_white), 1000*(end-start)))
        # condition = self.channels['laser'] == 255
        # cpy[condition] = 128

        print(len(cpy), len(cpy[0]))
        #
        # height, width, depth = cpy.shape
        # cpy[0:height, 0:width, 0:depth]
        # print(len(cpy), cpy[0][0])
